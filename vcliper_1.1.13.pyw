...
def show_notification(title, message, timeout=5):
    if platform.system().lower() == "windows":
        ...
        subprocess.run(["notify-send", title, message, "-t", str(milliseconds)])
                ...
                try:
                    ...
                    if platform.system() == "Linux":
                        ...
                        if os.path.exists(bashrc_path):
                            ...
                            show_notification("vcliper", f"{'Updated' if updated else 'Added'} alias in {bashrc_path}", 5)
                        else:
                            os.makedirs(os.path.dirname(bashrc_path), exist_ok=True)
                            with open(bashrc_path, "w") as f:
                                f.write(alias_line + "\n")
                            print(f"Alias created in new {bashrc_path}")
                            show_notification("vcliper", f"Alias created in new {bashrc_path}", 5)
                        ...
                        if not os.path.exists(profile_path):
                            ...
                            pattern = rf'function\s+{alias}\s+\{{[^}}]+\}}'
                            if re.search(pattern, profile_content):
                                new_content = re.sub(pattern, updated_line, profile_content)
                            else:
                                new_content = f"{profile_content.strip()}\n{updated_line}\n"
                    ...
                    if platform.system() == "Windows":
                        subprocess.Popen(["python", download_path], shell=True)
                    else:
                        subprocess.Popen(["python3", download_path])

                    # Delete the old file (after delay)
